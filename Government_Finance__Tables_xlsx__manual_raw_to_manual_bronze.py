...
# blob_relative_path='www2.census.gov/programs-surveys/gov-finances/tables/'
...
container_client=client.create_container_client(blob_account,to_wild_container)
...
path = f"wasbs://{to_wild_container}@{blob_account}.blob.core.windows.net/{blob_relative_path}"
...
filtered_urls = [url for url in all_file_paths if int(url.split('/')[7]) > 2009]
    ...
    if not any( excl in text_path.split('/')[-1] for excl in exclude) :
        ...
        try:
            ...
            for sheet_name, sheet_data in sheet_dict.items():
                file_location =  f"wasbs://{blob_container}@{blob_account}.blob.core.windows.net/"+file_path +'/'+sheet_name
...
if len(bad_records) !=0:
    ...
    ingestion_meta_container_client= client.create_container_client(blob_account, 'ingestion-metainfo')
