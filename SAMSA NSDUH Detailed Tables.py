...
# blob_relative_path ='www.samhsa.gov/data/sites/default/files/'
...
for files in all_file_paths :
    if files.split(files.split('/')[-1])[0] not in folder_paths :
        folder_paths.append(files.split(files.split('/')[-1])[0])
...
for folder in folder_paths :
    ...
    files = [blob.path.split('.net/')[-1]  for blob in files if blob.path.endswith('.htm') or blob.path.endswith('.html') ]
    for blob in files:
        try :
