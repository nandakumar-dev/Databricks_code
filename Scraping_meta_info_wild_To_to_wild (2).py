...
# MAGIC %md
# MAGIC This Notebook is for scraping file urls from the census html links and ingest the file urls in ingestion-meta container under scraped_urls directory in csv format
...
#getting blob path
file_path=manual_ingestion_meta_source_urls+input_Url_file_blob_path
...
def blob_write(Directory_List,part_links):
    ...
    if Directory_List is not None:
            ...
            blob_client = manual_ingestion_meta_container_client.get_blob_client(single_csv_filename)
            blob_client.upload_blob(output, overwrite=True)
            print('file_writen', single_csv_filename)
    else:
...
try:
    single_csv_filename = manual_ingestion_meta+common_blob_path+'/'+'DF/'
    ...
    file_location =  f'{common_blob_path}'+'/'+input_Url_file_blob_path
    blob_client = manual_ingestion_meta_container_client.get_blob_client(f"{file_location}")
...
#uploading error logs to the ingestion-metainfo container under bad_records directory
if not error_df.empty:
    bolb_path=input_Url_file_blob_path.replace('/','_').replace('.csv','')
    csv_filename = 'bad_records/'+ f"{bolb_path}_{run_datetime}.csv"
    output=error_df.to_csv(index=False)
    blob_client = manual_ingestion_meta_container_client.get_blob_client(csv_filename)
    blob_client.upload_blob(output, overwrite=True)
